...
export async function adaptPlan(supabase, userId) {
  ...
  const [{ data: progress }, { data: plan }] = await Promise.all([
    ...
    supabase.from('plans').select('ayah_per_day').eq('user_id', userId).order('created_at', { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  // RÈGLE 2 — Jours manqués sur les 7 derniers jours calendaires (L69)
  const last7Days = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date();
  ...
  // L68 — Limiter la variation à ±1 par adaptation
  newAyahPerDay = Math.max(currentAyahPerDay - 1, Math.min(currentAyahPerDay + 1, newAyahPerDay));

  // L72+L73+L74 — Toujours mettre à jour last_adaptation_date, en parallèle, avec try/catch
  try {
    if (newAyahPerDay !== currentAyahPerDay) {
      await Promise.all([
        supabase.from('progress').update({ ayah_per_day: newAyahPerDay, last_adaptation_date: today, last_adaptation_reason: reason }).eq('user_id', userId),
        supabase.from('plans').update({ ayah_per_day: newAyahPerDay }).eq('user_id', userId),
      ]);
    } else {
      await supabase.from('progress').update({ last_adaptation_date: today }).eq('user_id', userId);
    }
